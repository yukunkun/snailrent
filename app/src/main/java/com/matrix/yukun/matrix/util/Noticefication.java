...
import com.matrix.yukun.matrix.activity.SplashActivity;
...
public class Noticefication {
    public static void start(Context context) {
        ...
        // 显示在通知栏上的小图标
        mBuilder.setSmallIcon(R.mipmap.tool_icon_black);
        //通知标题
        mBuilder.setContentTitle("Matrix Photo");
        //通知内容
         mBuilder.setContentText("点击进入~");
        //设置大图标，即通知条上左侧的图片（如果只设置了小图标，则此处会显示小图标）
         mBuilder.setLargeIcon(BitmapFactory.decodeResource(context.getResources(), R.mipmap.tool_icon_black));
        //显示在小图标左侧的数字
